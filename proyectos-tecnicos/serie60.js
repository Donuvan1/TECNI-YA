function inicializarSerie60(configuracion) {
    console.log('Serie 60 cargada - Validaciones Parte 1');
    
    // ========== REFERENCIAS A ELEMENTOS ==========
    const anchoInf = document.getElementById('ancho_inf_s60');
    const anchoSup = document.getElementById('ancho_sup_s60');
    const altIzq = document.getElementById('alt_izq_s60');
    const altDer = document.getElementById('alt_der_s60');
    const hojasInput = document.getElementById('cant_hojas_s60');
    const fijasInput = document.getElementById('cant_fijas_s60');
    const corrOutput = document.getElementById('cant_corr_s60');
    const anchoSupAuto = document.getElementById('ancho_sup_auto_s60');

        // ========== FUNCIÃ“N PARA CALCULAR ANCHOS DE HOJAS ==========
    function calcularAnchosHojas(anchoTotal, hojas, fijas) {
        ...
        // Para 8 hojas (fÃ³rmula por definir, por ahora igual para todas)
        if (hojas === 8) {
    ...
    const incluirPuenteCheck = document.getElementById('incluir_en_calculos_s60');
    const altPuente = document.getElementById('alt_puente_s60');
    const spIzq = document.getElementById('sp_izq_s60');
    const spDer = document.getElementById('sp_der_s60');
    const espesor = document.getElementById('espesor_s60');
    const hojasPuente = document.getElementById('hojas_puente_s60');
    const fijosPuente = document.getElementById('fijos_puente_s60');
    const corrPuente = document.getElementById('corredizos_puente_s60');
    ...
    function cargarCostosAluminios() {
        ...
        const tipo = document.querySelector('input[name="opt_alum_s60"]:checked')?.value || 'estandar';
        const perfiles = configuracionCompleta.serie60?.perfiles?.[tipo] || [];
        const tbody = document.getElementById('tabla_precios_body_s60');
        
        if (tbody) {
            tbody.innerHTML = perfiles.map(p => `
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    <span><span class="num-fila">ðŸ“</span> ${p.n}</span>
    ...
    function cargarCostosAccesorios() {
        ...
        const tipo = document.querySelector('input[name="opt_alum_s60"]:checked')?.value || 'estandar';
        const accesorios = configuracionCompleta.serie60?.accesorios?.[tipo] || [];
        const tbody = document.getElementById('tabla_accesorios_body_s60');
        
        if (tbody) {
            tbody.innerHTML = accesorios.map(a => `
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    <span><span class="num-fila">ðŸ”§</span> ${a.n}</span>
    ...
    function cargarCostosTiras() {
        ...
        const tipo = document.querySelector('input[name="opt_alum_s60"]:checked')?.value || 'estandar';
        const tiras = configuracionCompleta.serie60?.tiras?.[tipo] || [];
        const tbody = document.getElementById('tabla_tiras_body_s60');
        
        if (tbody) {
            tbody.innerHTML = tiras.map(t => `
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    <span><span class="num-fila">ðŸ“</span> ${t.n}</span>
    ...
    function cargarCostosVidrios() {
        ...
        const vidrios = configuracionCompleta.serie60?.vidrios || [];
        const tbody = document.getElementById('tabla_vidrios_body_s60');
        
        if (tbody) {
            tbody.innerHTML = vidrios.map(v => `
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    <span><span class="num-fila">ðŸªŸ</span> ${v.n}</span>
    ...
    const switchMateriales = document.getElementById('switch_materiales_s60');
    if (switchMateriales) {
        switchMateriales.addEventListener('change', () => {
            toggleSeccion('switch_materiales_s60', 'seccion_materiales_s60');
            cargarCostosAluminios();
        });
    }
    
    const switchAccesorios = document.getElementById('switch_accesorios_s60');
    if (switchAccesorios) {
        switchAccesorios.addEventListener('change', () => {
            toggleSeccion('switch_accesorios_s60', 'seccion_accesorios_s60');
            cargarCostosAccesorios();
        });
    }
    
    const switchTiras = document.getElementById('switch_tiras_s60');
    if (switchTiras) {
        switchTiras.addEventListener('change', () => {
            toggleSeccion('switch_tiras_s60', 'seccion_tiras_s60');
            cargarCostosTiras();
        });
    }
    
        const switchVidrios = document.getElementById('switch_vidrios_s60');
    if (switchVidrios) {
        switchVidrios.addEventListener('change', () => {
            toggleSeccion('switch_vidrios_s60', 'seccion_vidrios_s60');
            cargarCostosVidrios();
            // Inicializar el selector de vidrios cuando se abre la secciÃ³n
            if (switchVidrios.checked) {
                setTimeout(() => {
                    inicializarSelectorVidriosS60();
                }, 100);
            }
        });
    }
    
    const radioEstandar = document.getElementById('alum_estandar_s60');
const radioPesados = document.getElementById('alum_pesados_s60');
...
if (radioPesados) {
    ...
    cargarCostosTiras();
    cargarCostosVidrios();
    // Inicializar selector de vidrios con bÃºsqueda
    function inicializarSelectorVidriosS60() {
    const buscador = document.getElementById('buscador_vidrio_s60');
    const listaContainer = document.getElementById('lista_vidrios_s60');
    
    if (!buscador) {
        console.log('Selector de vidrios no encontrado, reintentando en 500ms...');
        setTimeout(inicializarSelectorVidriosS60, 500);
        return;
    }
        ...
        cargarListaVidriosS60(buscador.value);
    });
    ...
    buscador.addEventListener('input', (e) => {
        ...
        if (texto === buscador.dataset.selectedValue) {
            buscador.value = '';
            cargarListaVidriosS60('');
        } else {
            cargarListaVidriosS60(texto);
        }
    });
    ...
    const vidrios = configuracionCompleta?.serie60?.vidrios || [];
    if (vidrios.length > 0) {
        seleccionarVidrioS60(0);
    }
}
    
    // ========== FUNCIÃ“N PARA VALIDAR COLOR ROJO ==========
    ...
    // ========== VALIDAR DIFERENCIA DE ALTURAS (mÃ¡x 1 cm, o uno es 0) ==========
    function validarDiferenciaAlturas() {
        ...
        if (esInvalido) {
            altIzq.title = "Diferencia mÃ¡xima 1 cm (o un lado en 0)";
            altDer.title = "Diferencia mÃ¡xima 1 cm (o un lado en 0)";
        } else {
            altIzq.title = "";
            altDer.title = "";
        }
    ...
    // ========== VALIDAR DIFERENCIA DE ANCHOS (mÃ¡x 5 cm, o uno es 0) ==========
    function validarDiferenciaAnchos() {
        ...
        if (esInvalido) {
            anchoInf.title = "Diferencia mÃ¡xima 5 cm (o un lado en 0)";
            anchoSup.title = "Diferencia mÃ¡xima 5 cm (o un lado en 0)";
        } else {
            anchoInf.title = "";
            anchoSup.title = "";
        }
    ...
    function validarHojas() {
        ...
        const permitidos = [1, 2, 3, 4, 6];  // â† 8 ya no estÃ¡ permitido
        ...
        if (esInvalido && valor !== 0) {
            if (valor === 5) {
                valor = 4;
                mostrarAlerta(`âš ï¸ Cantidad de hojas no permitida. Se ha ajustado a ${valor}. Permitidas: 1,2,3,4,6`);
            } else if (valor === 7) {
                valor = 6;
                mostrarAlerta(`âš ï¸ Cantidad de hojas no permitida. Se ha ajustado a ${valor}. Permitidas: 1,2,3,4,6`);
            } else if (valor === 8) {
    ...
    function validarFijas() {
        ...
        // Definir fijas permitidas segÃºn cantidad de hojas
        switch(hojas) {
            case 1:
                fijasPermitidas = [1];
                break;
        ...
        // Si no es vÃ¡lido, corregir al primer valor permitido
        if (!fijasValidas && fijasPermitidas.length > 0) {
            fijas = fijasPermitidas[0];
            fijasInput.value = fijas;
        }
        
        // Poner en rojo si es invÃ¡lido (igual que los otros campos)
    ...
    function actualizarAnchoAuto() {
        ...
        const anchoSpan = document.getElementById('ancho_actual_sugerencia_s60');
    ...
    function sugerirFijasSobreVentana() {
        ...
        const inputFijos = document.getElementById('fijos_solo_puente_s60');
        const sugerenciaSpan = document.getElementById('sugerencia_fijos_texto_s60');
        ...
        if (sugerenciaSpan) sugerenciaSpan.innerText = sugerido + " (automÃ¡tico)";
    }
    ...
    const contenedor = document.getElementById('vista_previa_hojas_s60');
    ...
    const corredizas = hojas - fijas;
        ...
        // Para mantener compatibilidad con el resto del cÃ³digo
    ...
    if (hayDescuadreAlturas && alturaMayor > 0) {
        // Para descuadre, tambiÃ©n aplicar la regla especial
    ...
    let anchoFijaAbajo = anchoFija;  // â† usar anchoFija real
let anchoFijaArriba = anchoFija;  // â† usar anchoFija real
...
let anchoAbajoCorrediza = anchoPorHoja;
    ...
    if (textoInfo) {
        html += `<div class="text-center text-info mb-2" style="font-size: 0.85rem;">ðŸ“Š ${textoInfo}</div>`;
    }
...
// ========== FUNCIONES PARA EL SELECTOR DE VIDRIOS CON BÃšSQUEDA ==========
    let vidrioSeleccionadoIndexS60 = 0;
    let vidriosListaS60 = [];
    
    function cargarListaVidriosS60(filtro = '') {
    const listaContainer = document.getElementById('lista_vidrios_s60');
    ...
    listaContainer.innerHTML = vidriosFiltrados.map((v) => {
        ...
        const isSelected = (originalIndex === vidrioSeleccionadoIndexS60);
        return `
    ...
    listaContainer.querySelectorAll('.vidrio-item').forEach(div => {
        div.addEventListener('click', (e) => {
            ...
            seleccionarVidrioS60(index);
            listaContainer.style.display = 'none';
            document.getElementById('buscador_vidrio_s60').value = vidrios[index].n;
        });
    });
    ...
    function seleccionarVidrioS60(index) {
    vidrioSeleccionadoIndexS60 = index;
    const vidrios = configuracionCompleta?.serie60?.vidrios || [];
    ...
    if (vidrio) {
        ...
        if (tbody) {
            tbody.innerHTML = `
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    <span><span class="num-fila">ðŸªŸ</span> ${vidrio.n}</span>
                    <span class="corte-medida">S/ ${vidrio.p.toFixed(2)} / P2</span>
                </li>
            `;
        }
        
        const switchVidriosReq = document.getElementById('switch_vidrios_req_s60');
    ...
    buscador.addEventListener('input', (e) => {
        ...
        cargarListaVidriosS60(e.target.value);
    });
...
function renderVidriosRequeridos() {
    const contenedor = document.getElementById('lista_vidrios_req_body_s60');
    if (!contenedor) return;
    
    // Obtener valores actuales
    const anchoInf = parseFloat(document.getElementById('ancho_inf_s60').value) || 0;
    const anchoSup = parseFloat(document.getElementById('ancho_sup_s60').value) || 0;
    const altIzq = parseFloat(document.getElementById('alt_izq_s60').value) || 0;
    const altDer = parseFloat(document.getElementById('alt_der_s60').value) || 0;
    const hojas = parseInt(document.getElementById('cant_hojas_s60').value) || 2;
    const fijas = parseInt(document.getElementById('cant_fijas_s60').value) || 0;
    const corredizas = hojas - fijas;
    ...
    // Calcular anchos por hoja usando la funciÃ³n existente
    let anchoHojaInf = 0;
    let anchoHojaSup = 0;
    ...
    // Determinar distribuciÃ³n de fijas (izquierda y/o derecha)
    ...
    if (configuracionCompleta?.serie60?.vidrios && vidrioSeleccionadoIndexS60 !== undefined) {
        vidrioSeleccionado = configuracionCompleta.serie60.vidrios[vidrioSeleccionadoIndexS60];
    }
    if (!vidrioSeleccionado && configuracionCompleta?.serie60?.vidrios?.length > 0) {
        vidrioSeleccionado = configuracionCompleta.serie60.vidrios[0];
    }
    ...
    // FunciÃ³n para redondear a mÃºltiplo de 5 superior
    function redondearMultiplo5(valor) {
        return Math.ceil(valor / 5) * 5;
    }
    
    // FunciÃ³n para calcular costo de un vidrio
    ...
    // ColecciÃ³n de vidrios por tipo
        ...
        html += `
            <li class="list-group-item d-flex align-items-center py-2">
                ...
                <div class="col-4 text-center">
                    ...
                    <div class="small">â†’ redondeado: ${anchoRedondeado} x ${altoRedondeado}</div>
    ...
    // Actualizar resumen si estÃ¡ visible
    const switchResumen = document.getElementById('switch_resumen_s60');
...
function renderAccesoriosRequeridos() {
    const contenedor = document.getElementById('lista_accesorios_req_body_s60');
    if (!contenedor) return;
    ...
    // Obtener modelo actual para cierre automÃ¡tico y manija
    let modeloActual = '';
    ...
    const listaCortes = document.getElementById('lista_cortes_detallada_s60');
    if (listaCortes) {
    ...
    const tipoAluminio = document.querySelector('input[name="opt_alum_s60"]:checked')?.value || 'estandar';
    const accesorios = configuracionCompleta?.serie60?.accesorios?.[tipoAluminio] || [];
    ...
    const cantidades = [
        ...
        cortaviento,        // CORTAVIENTO Serie 60
        guiaPlastica,       // GUIA PLASTICA Serie 60
...
function calcularFelpaHermetik() {
    ...
    // Determinar distribuciÃ³n de fijas
    ...
    if (corredizas > 0) {
        ...
        let felpaPorHoja;
        if (hayDescuadreAnchos) {
            // (4 Ã— altura) + (2 Ã— ancho_inf) + (2 Ã— ancho_sup)
            felpaPorHoja = (4 * altura) + (2 * ancho) + (2 * anchoSuperior);
        } else {
            // (4 Ã— altura) + (4 Ã— ancho)
...
function renderTirasRequeridas() {
    const contenedor = document.getElementById('lista_tiras_req_body_s60');
    if (!contenedor) return;
    ...
    html += `<li class="list-group-item bg-dark border-bottom border-info"><div class="d-flex justify-content-between"><span class="text-info fw-bold">ðŸ”¹ FELPA HERMETIK</span><span class="badge bg-info">Total: ${felpa.totalMetros.toFixed(2)} m</span></div></li>`;
    ...
    html += `<li class="list-group-item bg-dark border-bottom border-info mt-2"><div class="d-flex justify-content-between"><span class="text-info fw-bold">ðŸ”¸ BURLETE CUÃ‘A</span><span class="badge bg-info">Total: ${burlete.totalMetros.toFixed(2)} m</span></div></li>`;
    ...
    html += `<li class="list-group-item d-flex justify-content-between align-items-center mt-2 border-top border-warning bg-dark">
        <span class="text-warning fw-bold">ðŸ“ TOTAL TIRAS (FELPA + BURLETE):</span>
...
function mostrarMedidasGenerales() {
    const contenedor = document.getElementById('lista_cortes_detallada_s60');
    if (!contenedor) return;
    ...
    // ========== GENERAR LAS 21 LÃNEAS ==========
    const items = [];
    let contador = 0;
    
    // ========== DETERMINAR QUÃ‰ MARCO SEGÃšN MODELO ==========
...
// Determinar el modelo actual segÃºn hojas y fijas
let modeloActual = '';
if (hojas === 1 && fijas === 1) modeloActual = 'F';
else if (hojas === 2 && fijas === 0) modeloActual = 'CC';
else if (hojas === 2 && fijas === 1) modeloActual = 'FC';
...
// Calcular cantidades segÃºn modelo
...
// 1-4. MARCO DOBLE (6001)
contador++;
items.push({ num: contador, nombre: 'MARCO DOBLE', codigo: '6001', medida: anchoInf, cantidad: cantidadMarcoDoble });

contador++;
items.push({ num: contador, nombre: 'MARCO DOBLE', codigo: '6001', medida: hayDescuadreAnchos ? anchoSup : anchoInf, cantidad: cantidadMarcoDoble });

contador++;
items.push({ num: contador, nombre: 'MARCO DOBLE', codigo: '6001', medida: altIzq, cantidad: cantidadMarcoDoble });

contador++;
items.push({ num: contador, nombre: 'MARCO DOBLE', codigo: '6001', medida: hayDescuadreAlturas ? altDer : altIzq, cantidad: cantidadMarcoDoble });

// 5-8. MARCO TRIPLE (6002)
contador++;
items.push({ num: contador, nombre: 'MARCO TRIPLE', codigo: '6002', medida: anchoInf, cantidad: cantidadMarcoTriple });

contador++;
items.push({ num: contador, nombre: 'MARCO TRIPLE', codigo: '6002', medida: hayDescuadreAnchos ? anchoSup : anchoInf, cantidad: cantidadMarcoTriple });

contador++;
items.push({ num: contador, nombre: 'MARCO TRIPLE', codigo: '6002', medida: altIzq, cantidad: cantidadMarcoTriple });

contador++;
items.push({ num: contador, nombre: 'MARCO TRIPLE', codigo: '6002', medida: hayDescuadreAlturas ? altDer : altIzq, cantidad: cantidadMarcoTriple });
    
    // 9. Parante de corrediza
    contador++;
    items.push({ num: contador, nombre: 'PARANTE CORREDIZA', codigo: '6005', medida: hayDescuadreAlturas ? alturaCorredizaIzq : alturaCorredizaBase, cantidad: (corredizas * 2) * mult });
    
    // 10. Marco inf corrediza
    contador++;
    items.push({ num: contador, nombre: 'MARCO INF CORREDIZA', codigo: '6005', medida: anchoHojaInf, cantidad: corredizas * mult });
    
    // 11. Marco sup corrediza
    contador++;
    items.push({ num: contador, nombre: 'MARCO SUP CORREDIZA', codigo: '6005', medida: anchoHojaSup, cantidad: corredizas * mult });
    
    // 12. Parante fija izquierda
    contador++;
    items.push({ num: contador, nombre: 'PARANTE FIJA IZQUIERDA', codigo: '6005', medida: hayDescuadreAlturas ? alturaFijaIzq : alturaFija, cantidad: tieneFijaIzq ? 2 * mult : 0 });
    
    // 13. Parante fija derecha
    contador++;
    items.push({ num: contador, nombre: 'PARANTE FIJA DERECHA', codigo: '6005', medida: hayDescuadreAlturas ? alturaFijaDer : alturaFija, cantidad: tieneFijaDer ? 2 * mult : 0 });
    ...
    contador++;
    items.push({ num: contador, nombre: 'MARCO INF FIJA IZQ', codigo: '6005', medida: anchoFijaInf, cantidad: tieneFijaIzq ? 1 * mult : 0 });
    
    // 15. Marco sup fija izquierda
    contador++;
    items.push({ num: contador, nombre: 'MARCO SUP FIJA IZQ', codigo: '6005', medida: anchoFijaSup, cantidad: tieneFijaIzq ? 1 * mult : 0 });
    
    // 16. Marco inf fija derecha
    contador++;
    items.push({ num: contador, nombre: 'MARCO INF FIJA DER', codigo: '6005', medida: anchoFijaInf, cantidad: tieneFijaDer ? 1 * mult : 0 });
    
    // 17. Marco sup fija derecha
    contador++;
    items.push({ num: contador, nombre: 'MARCO SUP FIJA DER', codigo: '6005', medida: anchoFijaSup, cantidad: tieneFijaDer ? 1 * mult : 0 });
    ...
    contador++;
    items.push({ num: contador, nombre: 'TRASLAPE FIJA', codigo: '6003', medida: hayDescuadreAlturas ? alturaFija : alturaFija, cantidad: traslapeFija });
    
    // 19. Traslape de corrediza
    contador++;
    items.push({ num: contador, nombre: 'TRASLAPE CORREDIZA', codigo: '6003', medida: hayDescuadreAlturas ? alturaCorredizaBase : alturaCorredizaBase, cantidad: corredizas * mult });
    
    // 20. Traslape central
    contador++;
    items.push({ num: contador, nombre: 'TRASLAPE CENTRAL', codigo: '6003', medida: hayDescuadreAlturas ? alturaCorredizaBase : alturaCorredizaBase, cantidad: traslapeCentral * mult });
    
    // 21. Adaptador de hoja
    contador++;
    items.push({ num: contador, nombre: 'ADAPTADOR DE HOJA', codigo: '6004', medida: hayDescuadreAlturas ? alturaCorredizaBase : alturaCorredizaBase, cantidad: adaptador * mult });
...
function calcularAluminiosRequeridos() {
    ...
    // Tipos de aluminio (estÃ¡ndar o pesado)
    const tipoAluminio = document.querySelector('input[name="opt_alum_s60"]:checked')?.value || 'estandar';
    const perfiles = configuracionCompleta.serie60?.perfiles?.[tipoAluminio] || [];
        ...
        if (hayDescuadreAnchos || hayDescuadreAlturas) {
            ...
            if (hayDescuadreAlturas) {
                ...
                alturaHDer = Math.round(alturaHDer * 10) / 10;
                        ...
                        // Determinar quÃ© ancho usar para fijas
...
function mostrarAluminiosRequeridos() {
    ...
    const contenedor = document.getElementById('lista_requeridos_body_s60');
    if (!contenedor) return;
            ...
            <li class="list-group-item d-flex align-items-center py-2">
                <div class="col-5">
                    ...
                    <div class="small text-muted mt-1">
                        <span class="badge bg-secondary" style="font-family: monospace; font-size: 0.7rem;">CÃ³digo: ${item.codigo}</span>
                    </div>
                </div>
...
function actualizarTodo() {
    ...
    function manejarEnter(event) {
        if (event.key === 'Enter') {
            ...
            if (!validarDiferenciaAnchos() || !validarDiferenciaAlturas()) {
                mostrarAlerta('âš ï¸ Verifique las diferencias:\n- Alturas: mÃ¡ximo 1 cm (o un lado en 0)\n- Anchos: mÃ¡ximo 5 cm (o un lado en 0)');
            } else if (!validarMinimoAncho(anchoInf) || !validarMinimoAncho(anchoSup)) {
                mostrarAlerta('âš ï¸ Ancho mÃ­nimo: 40 cm');
            } else if (!validarMinimoAltura(altIzq) || !validarMinimoAltura(altDer)) {
                mostrarAlerta('âš ï¸ Altura mÃ­nima: 30 cm');
            } else if (!validarHojas()) {
                mostrarAlerta('âš ï¸ Hojas permitidas: 1, 2, 3, 4, 6 (8 en desarrollo)');
            }
        }
    }
    
    // ========== CONFIGURAR TOGGLE PUENTE ==========
    const btnToggle = document.getElementById('btn_toggle_vista_s60');
    const panelPuente = document.getElementById('seccion_puente_master_s60');
    ...
    const radioFijos = document.getElementById('radio_fijos_s60');
    const radioCorr = document.getElementById('radio_corr_s60');
    const configCorredizos = document.getElementById('config_corredizos_puente_s60');
    const configFijos = document.getElementById('config_fijos_puente_s60');
    ...
    // BotÃ³n sugerir fijos
    const btnSugerirFijos = document.getElementById('btn_sugerir_fijos_s60');
    if (btnSugerirFijos) {
    ...
    const switchReq = document.getElementById('switch_req_s60');
    if (switchReq) {
        switchReq.addEventListener('change', () => {
            const seccion = document.getElementById('resultados_requeridos_s60');
            if (seccion) {
    ...
    // TambiÃ©n cuando cambia el tipo de aluminio (estÃ¡ndar/pesado)
    const radioEstandarReq = document.getElementById('alum_estandar_s60');
    const radioPesadosReq = document.getElementById('alum_pesados_s60');
    if (radioEstandarReq) {
    ...
    const switchCortes = document.getElementById('switch_cortes_s60');
    if (switchCortes) {
        switchCortes.addEventListener('change', () => {
            const seccion = document.getElementById('resultados_cortes_s60');
            if (seccion) {
...
if (switchVidriosReq) {
    switchVidriosReq.addEventListener('change', () => {
        const seccion = document.getElementById('resultados_vidrios_req_s60');
        if (seccion) {
...
// TambiÃ©n cuando cambia el tipo de aluminio (estÃ¡ndar/pesado) - afecta alturas de hojas
if (radioEstandar) {
    radioEstandar.addEventListener('change', () => {
        if (switchVidriosReq && switchVidriosReq.checked) {
            renderVidriosRequeridos();
...
const switchAccesoriosReq = document.getElementById('switch_accesorios_req_s60');
if (switchAccesoriosReq) {
    switchAccesoriosReq.addEventListener('change', () => {
        const seccion = document.getElementById('resultados_accesorios_req_s60');
        if (seccion) {
...
const switchTirasReq = document.getElementById('switch_tiras_req_s60');
if (switchTirasReq) {
    switchTirasReq.addEventListener('change', () => {
        const seccion = document.getElementById('resultados_tiras_req_s60');
        if (seccion) {
...
// ========== RENDERIZAR RESUMEN DE PRODUCCIÃ“N ==========
// ========== RENDERIZAR RESUMEN DE PRODUCCIÃ“N (COMPACTO) ==========
function renderResumenS60() {
    const contenedor = document.getElementById('contenedor_resumen_s60');
    if (!contenedor) return;
    ...
    // ========== ALUMINIOS ==========
    const listaAluminios = document.getElementById('lista_requeridos_body_s60');
    if (listaAluminios) {
        let items = [];
        listaAluminios.querySelectorAll('li').forEach(item => {
            ...
            let codigo = codigoSpan ? codigoSpan.innerText.replace('CÃ³digo:', '').trim() : '';
    ...
    // ========== VIDRIOS ==========
    const listaVidrios = document.getElementById('lista_vidrios_req_body_s60');
    if (listaVidrios) {
        let items = [];
    ...
    // ========== ACCESORIOS ==========
    const listaAccesorios = document.getElementById('lista_accesorios_req_body_s60');
    if (listaAccesorios) {
        let items = [];
    ...
    const tipoAluminio = document.querySelector('input[name="opt_alum_s60"]:checked')?.value || 'estandar';
    const tirasConfig = configuracionCompleta?.serie60?.tiras?.[tipoAluminio] || [];
    const precioFelpa = tirasConfig.find(t => t.n === 'felpa hermetik')?.p || 0;
const precioBurlete = tirasConfig.find(t => t.n === 'burlete cuÃ±a S-60')?.p || 0;
    
    // ========== LEER SUBTOTALES DE TIRAS REQUERIDAS ==========
    let totalFelpaMetros = 0;
    let totalBurleteMetros = 0;
    
    const listaTiras = document.getElementById('lista_tiras_req_body_s60');
    if (listaTiras) {
    ...
    if (totalBurleteMetros > 0) {
        tirasItems.push(`Burlete CuÃ±a           bu-3mm    ${totalBurleteMetros.toFixed(2)} m   S/. ${totalBurleteSoles.toFixed(2)}`);
...
if (switchResumen) {
    switchResumen.addEventListener('change', () => {
        const seccion = document.getElementById('resultados_resumen_s60');
        if (seccion) {
...
// BotÃ³n Refrescar (si existe)
const btnRefrescar = document.querySelector('#resultados_resumen_s60 .btn-outline-info');
if (btnRefrescar) {
...
// BotÃ³n Imprimir (si existe)
const btnImprimir = document.querySelector('#resultados_resumen_s60 .btn-outline-secondary');
if (btnImprimir) {
    btnImprimir.addEventListener('click', () => {
        const contenido = document.getElementById('contenedor_resumen_s60')?.innerText;
        if (!contenido) {
        ...
        ventana.document.write(`
            ...
            <html>
            <head>
                <title>Resumen de ProducciÃ³n - Serie 60</title>
                <style>
    ...
    // ========== INICIALIZAR ==========
    console.log('Serie 60 inicializada - Validaciones activas');
    sugerirHojasPorAncho();
    actualizarCorredizas();
    actualizarAnchoAuto();
    actualizarCorrPuente();
    ...
    window.renderResumenS60 = renderResumenS60;
}

function setMaterialReqS60(tipo) {
    const radioEstandar = document.getElementById('alum_estandar_s60');
    const radioPesados = document.getElementById('alum_pesados_s60');
    const btnEst = document.getElementById('btn_req_estandar_s60');
    const btnPes = document.getElementById('btn_req_pesado_s60');
